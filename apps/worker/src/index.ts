  ...
  async runAnalysis(symbolOverride?: string): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      const markedPositions = existingPositions.map((position) => {
        ...
        if (!position.tp1Taken && hitFirstTarget && targets.length > 1) { const partialSize = size / 2; const partialPnl = ((isLong ? price - entry : entry - price) * partialSize) - price * partialSize * closeFeePct; journal.unshift({ timestamp: Date.now(), symbol, category: "PAPER_TRADE", title: "Partial take-profit", body: `Half position closed for ${partialPnl.toFixed(2)}`, data: { pnl: partialPnl } }); return { ...position, positionSize: partialSize, notional: price * partialSize, currentPrice: price, realizedPnl: Number(position.realizedPnl ?? 0) + partialPnl, tp1Taken: true, takeProfits: targets.slice(1) }; }
